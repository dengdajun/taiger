 ...
 */	
	...
	function saveD(){
		...
		var fields = dname + ',guardian_angel,' + dtype + ',guardian_angel,' + dbz + ',guardian_angel,' + isQian + ',guardian_angel,' + ddefault + ',guardian_angel,' + flength;;
	...
	function editField(value,msgIndex){
		$("#dialog-add").css("display","block");
		var efieldarray = value.split(',guardian_angel,');
	...
	function appendC(value){
		var fieldarray = value.split(',guardian_angel,');
		$("#fields").append(
			'<tr>'+
	...
	function isSame(value){
		for(var i=0;i<arField.length;i++){
			var array0 = arField[i].split(',guardian_angel,')[0];
			if(array0 == value){
				return false;
			}
		}
